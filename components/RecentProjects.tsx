...
const ProjectCard = ({ project, className = "", minHeight }: { project: Project; className?: string; minHeight?: number }) => {
  // Calculate dynamic height based on description length
  const baseHeight = 32; // 32rem base height
  const additionalHeight = Math.max(0, Math.ceil((project.des.length - 100) / 80) * 3); // Add height for longer descriptions
  const calculatedHeight = baseHeight + additionalHeight;
  const finalHeight = minHeight ? Math.max(calculatedHeight, minHeight) : calculatedHeight;
  ...
  return (
    <motion.div
      ...
      className={`w-full max-w-[90vw] sm:max-w-[32rem] mx-auto flex flex-col items-center justify-center mb-8 ${className}`}
      style={{ height: `${finalHeight}rem` }}
    >
      <PinContainer title={project.link} href={project.link}>
        ...
        <div className="text-center w-full px-4 flex-1 flex flex-col justify-between">
          <div>
            ...
            <p className="lg:text-xl md:text-base text-sm font-light text-white/70 group-hover:text-white/90 transition-colors duration-300 my-3">
              {project.des}
            </p>
          </div>
...
const RecentProjects = ({ setIsAnyModalOpen }: RecentProjectsProps) => {
  ...
  // Calculate the maximum height needed for uniformity
  const getMaxHeight = (projectList: Project[]) => {
    return Math.max(...projectList.map(project => {
      const baseHeight = 32;
      const additionalHeight = Math.max(0, Math.ceil((project.des.length - 100) / 80) * 3);
      return baseHeight + additionalHeight;
    }));
  };

  const recentMaxHeight = getMaxHeight(recentProjects);
  const allMaxHeight = getMaxHeight(projects);
